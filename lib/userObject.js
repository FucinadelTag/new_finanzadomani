...
class userObject {
    ...
    getNome () {
        if (this.userData.nome){
            ...
            return this.userData.auth0.name.givenName;
        }
    }

    getNomeCarta () {
        return this.userData.nome_carta;
    }

    getIndirizzoFatturazione () {
        return this.userData.indirizzo_fatturazione;
    }

    getCognome () {

        if (this.userData.cognome){
            return this.userData.cognome
        } else {
            return this.userData.auth0.name.familyName;
        }
    }
    ...
    getEmail () {
        if (this.userData.email){
            ...
            return this.userData.auth0._json.email;
        }
    }
    ...
    async updateStripeUserCreditCard (userOk){
        let newStripeUser = await stripe.customers.update(userOk.stripeId, {
            source: userOk.stripeToken
        });

        let user = this.saveStripeUser (newStripeUser);
    ...
    getDatiCartaCredito (){
        if (this.userData.stripeToken){
            return (this.userData.stripeUser.sources.data[0]);
        }
